    ...
    <div className="scanner-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      {error ? (
        <div className="bg-red-500/20 border border-red-400 text-white rounded-xl p-4 text-center max-w-xs">
          ...
          <p>{error}</p>
          <button onClick={onClose} className="mt-3 bg-white/20 px-4 py-2 rounded-lg text-sm">إغلاق</button>
        </div>
      ) : (
        <>
      ...
      <button onClick={onClose} className="mt-6 bg-white/10 hover:bg-white/20 text-white px-8 py-3 rounded-xl font-medium transition-colors">
        إغلاق
      </button>
    </div>
  )
